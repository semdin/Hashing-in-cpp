...
void insertIntoHashTable(const char* input) {
	...
	string s = input;
	int intS = 0; //new input

	for(int n=0; n<s.size(); n++){

        //cout<<(int)s[n];
        intS += ((int)s[n]) * pow(10, 2*n);
	}


	int hashAddress = intS % 10;

	if (!searchInHashTable(input)) {
        hashTable[hashAddress].addToTail(intS);
...
bool searchInHashTable(int input) {
	...
	if(hashTable[hash].isInList(input))
        return true;
    else
        return false;
}
...
bool searchInHashTable(const char* input) {
	...
	if(hashTable[hash].isInList(intS))
        return true;
    else
        return false;
...
int main() {
    ...
    insertIntoHashTable("A");
	insertIntoHashTable("AAA");
    insertIntoHashTable(65);
